...
export const magic_words = async (container: Container): Promise<Scene> => {
  ...
  emoji_textures.push({
    ...
    promise: fetch_texture(
      "https://api.dicebear.com/9.x/fun-emoji/png?seed=Chase"
    ),
  });
  ...
  await Promise.all(character_textures.map((v) => v.promise)).then(
    (fetched_textures) => {
  ...
  await Promise.all(emoji_textures.map((v) => v.promise)).then(
    (fetched_textures) => {
  ...
  data.dialogue.forEach((message: any, index: number) => {
    ...
    char_sprite.x =
      characters[message.name].position === "left"
        ? message_left - avatar_w2
        : message_right + 20;
